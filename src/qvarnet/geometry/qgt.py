...
def compute_natural_gradient_minsr(params, batch, e_loc, model_apply, qgt_config: QGTConfig):
    ...
    Returns:
        (natural_grad_flat, unravel_fn) — same convention as ``compute_natural_gradient``.
    """
    flat_params, unravel_fn = ravel_pytree(params)
    ...
    T = (O_bar @ O_bar.T) / M + qgt_config.regularization * jnp.eye(M)
    y = jax.scipy.linalg.cho_solve(jax.scipy.linalg.cho_factor(T), e)  # T⁻¹ e, (M,)
    natural_grad = (2.0 / M) * (O_bar.T @ y)  # (P,)
    return natural_grad, unravel_fn
