...
def compute_qgt(params, batch, model_apply, regularization=1e-6):
    ...
    correlation_matrix = jnp.mean(
        jnp.einsum('bi,bj->bij', O_i_conj, O_i), axis=0
    )
...
def solve_qgt_cholesky(S, grads):
    ...
    try:
        ...
        return natural_grad
    except jax.linalg.LinAlgError as e:
        warnings.warn(f"Cholesky decomposition failed: {e}. Falling back to direct solve.")
...
def train_step_qgt(state, batch, qgt_config):
    ...
    new_params_flat = flatten_params(state.params)[0] - learning_rate * natural_grad_flat
...
def compute_qgt_block_diagonal(params, batch, model_apply, layer_sizes, regularization=1e-6):
    """
