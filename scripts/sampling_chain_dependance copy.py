...
number_of_chains = range(N_MAX, N_MAX + 1)
...
for n_chain in number_of_chains:
    print(f"\n\nRunning sampling with {n_chain} chains...\n")
    ...
    print(
        f"\n\t**Total time for sampling with {n_chain} chains: {end_global_timer - start_global_timer:.2f} seconds.**"
    )
...
with open(
    f"results/{device}/times_with_{number_of_steps_per_chain}_split_{split}.csv",
    mode="w",
    newline="",
) as file:
