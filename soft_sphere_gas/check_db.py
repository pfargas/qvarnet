...
Eq.31 value stored at run time, prints the queue status and any failed runs, and shows the
seed-aggregated curve (what ``analysis.load_curve`` feeds the plot).

    python check_db.py                       # default outputs/runq.db
    python check_db.py --db ~/dilute-bose/outputs/runq.db
"""
...
from analysis import load_curve
...
def main():
    ...
    print(f"=== {args.db} ===")
    print(f"queue status: {store.status_counts(conn)}\n")

    rows = [
        {**json.loads(r["params_json"]), **json.loads(r["result_json"])}
        for r in store.fetch(conn, "done")
    ]
    rows.sort(key=lambda r: (r["R"], r["x"], r["N"], r["seed"]))
    if not rows:
    ...
    hdr = (f"{'R':>5} {'x':>10} {'N':>4} {'seed':>4} {'E/N':>11} {'err':>9} "
    ...
    for r in rows:
        ...
        print(f"{R:>5g} {x:>10.3e} {N:>4} {r['seed']:>4} {e:>11.4e} {err:>9.2e} "
              f"{lb:>10.4e} {ly:>10.4e} {ub:>10.4e} {e/ly:>6.2f} {tag:>9} "
              f"{verdict:>7} {r.get('epochs_ran', '?'):>7}")
...
def _print_failures(conn):
    fails = store.fetch(conn, "failed")
    if fails:
        print(f"\n{RED}failed runs:{RESET}")
        for f in fails:
            p = json.loads(f["params_json"])
            line1 = (f["error"] or "").strip().splitlines()[-1:] or [""]
            print(f"  SS{p['R']:g} x={p['x']:g} N={p['N']} seed={p['seed']}: {line1[0]}")
    skipped = store.fetch(conn, "skipped")
    if skipped:
        print(f"\n{DIM}{len(skipped)} skipped (box-infeasible) point(s){RESET}")


def _print_curve(conn, rows):
    R_Ns = sorted({(r["R"], r["N"]) for r in rows})
    for R, N in R_Ns:
        try:
            c = load_curve(conn, R, N)
        except Exception as exc:
            print(f"\nSS{R:g} N={N}: load_curve failed: {exc!r}")
            continue
        if len(c["x"]) == 0:
            continue
        print(f"\nseed-aggregated curve (SS{R:g}, N={N}, verdict-passing): "
